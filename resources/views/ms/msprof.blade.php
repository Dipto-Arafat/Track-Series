...
<body>
    ...
    <div class="msdashboard">
        <fieldset style='margin-left:10px;margin-top:10px;margin-right:10px;margin-bottom: 10px'>
            <img alt='MOVIE/SERIES PICTURE' class='followerimage' src='{{asset('upload')}}/{{$msinfo->ms_picture}}'
                    width='500px'
                    height='250px' style='display: block'>
                    <br>
                    <h3>{{$msinfo->ms_name}}<h3>
                            @if($followflag == false)
                            <input type='button' class='signbutton' id='{{$msinfo->ms_id}}' value='FOLLOW'
                                onclick='followms(this)'>
		                    @else
                            <input type='button' class='signbutton' id='{{$msinfo->ms_id}}' value='UNFOLLOW'
    ...
    @endphp
            ...
            @for($intg = 1;$intg <= 5;$intg++)
				@if($intg <= $myrating)
                                <span class='fa fa-star' id='{{Session::get('xuser')->user_id}}*{{$msinfo->ms_id}}*{{$intg}}'
			...
			@endfor
                            ...
                            <input type='button' style='margin-left: 110px;margin-bottom: 10px' class='signbutton'
                                id='{{$msinfo->ms_id}}' value='POST' onclick='updateReview(this)'>
                            </fieldset>
        </div>
            ...
            <fieldset style='margin-left:30px;margin-right:30px;margin-top:20px;margin-bottom:20px'>
                <h4 style='font-weight:bold;font-family:cursive;text-align: center'>RATING<h4>
                        <div class="row">
                            ...
                            <div class="middle">
                                <div class="bar-container">
                                    <div style='width:@php if($total == 0){echo '0%';}else{ $ops = $five/$total; $ops = round($ops * 100); echo $ops.'%';} @endphp;height: 18px; background-color: #4CAF50;'>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="middle">
                            <div class="bar-container">
                                <div
                                    style='width:@php if($total == 0){echo '0%';}else{ $ops = $four/$total; $ops = round($ops * 100); echo $ops.'%';} @endphp; height: 18px; background-color: #2196F3;'>
                                </div>
                            </div>
                        </div>
                        <div class="side right">
                            <div>{{$four}}</div> </div> <div class="side">
                                        <div>3 star</div>
                                    </div>
                                    <div class="middle">
                                        <div class="bar-container">
                                            <div style='width:@php if($total == 0){echo '0%';}else{ $ops = $three/$total; $ops = round($ops * 100); echo $ops.'%';} @endphp;height: 18px; background-color: #00bcd4;'>
                                </div>
                            </div>
                        </div>
                        <div class="side right">
                        ...
                        <div class="middle">
                            <div class="bar-container">
                                <div
                                    style='width:@php if($total == 0){echo '0%';}else{ $ops = $two/$total; $ops = round($ops * 100); echo $ops.'%';} @endphp; height: 18px; background-color: #ff9800;'>
                                </div>
                            </div>
                        </div>
                        <div class="side right">
                            <div>{{$two}}</div> </div> <div class="side">
                                                <div>1 star</div>
                                            </div>
                                            <div class="middle">
                                                <div class="bar-container">
                                                    <div style='width:@php if($total == 0){echo '0%';}else{ $ops = $one/$total; $ops = round($ops * 100); echo $ops.'%';} @endphp; height: 18px; background-color: #f44336;'>
                                </div>
                            </div>
                        </div>
                        <div class="side right">
                ...
                <h4 style='font-weight:bold;font-family:cursive'>DETAILS<h4>
                        <table class="user_table" height="400" width="500" align="center">
                            <tr>
                                ...
                                <td><b class="table_data">{{$msinfo->ms_name}}</b></td>
                            </tr>
                            <tr>
                                <td><b class="table_data">CATEGORY</b></td>
                                <td><b class="table_data">{{$msinfo->ms_cat}}</b></td>
                            </tr>
                            <tr>
                                <td><b class="table_data">DIRECTOR</b></td>
                                <td><b class="table_data">{{$msinfo->ms_director}}</b></td>
                            </tr>
                            <tr>
                                <td><b class="table_data">ACTOR</b></td>
                                <td><b class="table_data">{{$msinfo->ms_actor}}</b></td>
                            </tr>
                            <tr>
                                <td><b class="table_data">ACTRESS</b></td>
                                <td><b class="table_data">{{$msinfo->ms_actress}}</b></td>
                            </tr>
                            <tr>
                                <td><b class="table_data">RELEASE DATE</b></td>
                                <td><b class="table_data">{{$msinfo->ms_release_date}}</b></td>
                            </tr>
                            <tr>
                                <td><b class="table_data">REGION</b></td>
                                <td><b class="table_data">{{$msinfo->ms_country}}</b></td>
                            </tr>
                        </table>
            </fieldset>

            <fieldset style='margin-left:30px;margin-right:30px;margin-top:20px;margin-bottom:20px;text-align:center'>
                <h4 style='font-weight:bold;font-family:cursive'>REVIEWS({{count($pepreview)}})</h4>
                <hr>
                <fieldset>
                    <table class='signup_table' height='400' width='500' align='center'>

                        @for($k=0;$k<count($pepreview);$k++)
                        <tr>
                            <td style='text-align:left'><a href="/tsrm/userprof/{{$pepreview[$k]->user_id}}">
                                    {{$pepreview[$k]->fname}} {{$pepreview[$k]->lname}}</a><br>
                                @for($ii = 1;$ii <= 5;$ii++)
						@if($ii <= $pepreview[$k]->rating)
                        ...
                        @endif
                        @endfor
                                <h4>Review Date:{{$pepreview[$k]->date}}</h4>
                                <p>{{$pepreview[$k]->comment}}</p>
                                <hr width='100%'>
                            </td>
                        </tr>
                        @endfor
