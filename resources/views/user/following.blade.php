  ...
  <div class="main">
        ...
        <table class="table table-dark table-striped table-borderless table-hover table-sm">
          ...
          @if($data)
            ...
            <tr>
                <td > <img src="{{asset('upload')}}/{{$data[$a]->picture}}" alt="IMAGE" class="mr-3 mt-3 rounded-circle" style="width:60px;"></td>
                <td ><a href='{{route('tsrm.userprof',['id' => $data[$a]->user_id])}}'>{{$data[$a]->fname}} {{$data[$a]->lname}}</a></td>
                <td >{{$data[$a]->gender}}</td>
                <td ><a href="{{route('user.unfollowuser',['id' => $data[$a]->user_id])}}" class="badge badge-secondary">UNFOLLOW</a></td>
              </tr>
@endfor
@endif
        </table>
