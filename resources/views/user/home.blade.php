...
<body>
  ...
  <div class="main">
          ...
          <h4 style='text-align:center;color:white'>RECENT REVIEWS</h4>
              @for($p = 0;$p<count($alldata);$p++)
              <div class="media border p-1">
                  <img src="{{asset('upload')}}/{{$alldata[$p]->picture}}" alt="IMAGE" class="mr-3 mt-3 rounded-circle" style="width:60px;">
                  <div class="media-body" style='margin:1px'>
                    <h4 style='color:white'><a href='{{route('tsrm.userprof',['id' => $alldata[$p]->user_id])}}'>{{$alldata[$p]->fname}} {{$alldata[$p]->lname}} </a><small><i>Reviewed <a href='{{route('tsrm.msprof',['id' => $alldata[$p]->ms_id])}}'>{{$alldata[$p]->ms_name}}</a> on {{$alldata[$p]->date}}</i></small></h4>
                    @if($alldata[$p]->rating)
                    <p style='color:white;font-size:20px'>Rated:{{$alldata[$p]->rating}} Star!</p>
                    @endif
                    @if($alldata[$p]->comment)
                    <p style='color:white;font-size:15px'>{{$alldata[$p]->comment}}</p>
                    @endif      
                  </div>
                </div>       
                @endfor
          @else
              {{'Please Follow People To See Their Reviews'}}
          @endif 
  </div>
