...
.trackcontainer:hover input ~ .trackmark {
  background-color: #ccc;
}

/* When the checkbox is checked, add a blue background */
.trackcontainer input:checked ~ .trackmark {
...
.trackcontainer input:checked ~ .trackmark:after {
  display: block;
}

/* Style the checkmark/indicator */
.trackcontainer .trackmark:after {
  left: 13px;
  top: 10px;
  width: 15px;
  height: 20px;
...
<body>
  ...
  <div class="main">
    <h4 style='text-align:center;color:white'>TRACK EPISODE</h4>
    <form>
      <div class="form-row align-items-center justify-content-center">
        <div class="col-2 form-group">
          <select class="custom-select mt-3" id="mainselect">
            <?php if(count($data)>0): ?>
              <?php for($i = 0;$i<count($data);$i++): ?>
                <option value="<?php echo e($data[$i]->ms_id); ?>"><?php echo e($data[$i]->ms_name); ?></option>
              <?php endfor; ?>
            </select>
            <?php else: ?>
              Please Follow Series To Track The Episodes! Make It Easier!
            <?php endif; ?>  
            </div>
          </div>
      </form>
      <div id='trackcontainerepisode' style='align-items:center;margin-top:20px;margin-left:50px;margin-right:50px'></div>
  </div>
...
<script>
  ...
  $(document).ready(function () {
    ...
    $('#mainselect').change(function () {
      var catid = $('#mainselect').val();
      $.ajax({
        url: '/tsrm/ms/allepisode/'+catid,
        contentType: 'application/json',
        success: function (response) {
          document.getElementById('trackcontainerepisode').innerHTML = response;
        }
      });
    }).trigger("change");
...
*/
  ...
  function manageEpisode(v) {
    $.ajax({
        url: '/tsrm/user/episodeupdate/'+v.id+'/'+v.checked,
        contentType: 'application/json',
        success: function (response) {
          //alert(response);
        }
...
<?php /* F:\Spring 18-19\ATP 3\FINAL\Project\Dipto\Project V 29.4.19\resources\views/user/track.blade.php */ ?>