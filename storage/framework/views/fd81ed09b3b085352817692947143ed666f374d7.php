    ...
    ?>
			...
			<?php endfor; ?>
                            ...
                            <input type='button' style='margin-left: 110px;margin-bottom: 10px' class='signbutton'
                                id='<?php echo e(Session::get('xuser')->user_id); ?>' value='POST' onclick='updateReview(this)'>
                            </fieldset>
        </div>
        ...
        <script>
            ...
            updateops = new XMLHttpRequest();
            function updateReview(v) {
                alert(v.id);
                var m = document.getElementById('com').value;
                alert(m);
                updateops.onreadystatechange = function () {
                    if (updateops.readyState == 4 && updateops.status == 200) {
                        alert('if');
                        var resultText = updateops.responseText;
                        alert(resultText);
                        //document.getElementById('com').innerHTML=resultText;			
                        if (resultText == 'true') {
                            myFunction();
                        }
                    }
                };
                var url = "validation.php?updatecomment=" + v.id + "&comment=" + m;
                updateops.open("GET", url, true);
                updateops.send();
            }
...
<?php /* F:\Spring 18-19\ATP 3\FINAL\Project V 26.4.19 - Copy\resources\views/ms/msprof.blade.php */ ?>